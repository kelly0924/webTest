...
<%
    ...
    if(result.next()){//예외 처리 해줘서 로그인 이 안되면 빈 값 아니면 로그인  비여 있지 않으면  맞는지 등 다양한 처리 가능 
        ...
        if(targetId.equals(user_id) && targetPw.equals(user_pw)){//데이터 베이스에서 가져온 아이디와 유저가 입력한 아디가 같은 경우 즉  로그인 성공시 
            ...
            Cookie user=new Cookie("user_id",user_id);//cookie 생성 user_id라는 이름을 가진 쿠키를 생성 한다. 
            response.addCookie(user);//생성한 쿠키를 클라이언트로 응답해 줄겠다. 
            response.sendRedirect("main.jsp");
        }
    }else{
        response.sendRedirect("loginFrom.jsp");//로그인을 실패 했을때 페이지를 돌려 준다. response 돌려 주겠다. 
    }
